...
def datingClassTest():
    ...
    # 从文件中加载数据
    datingDataMat, datingLabels = file2matrix("../input/knn/datingTestSet2.txt")  # load data setfrom file
    #print(datingLabels)
    # 归一化数据
...
def handwritingClassTest():
    ...
    # 1. 导入数据
    hwLabels = []
    trainingFileList = os.listdir("../../input/knn/trainingDigits") # load the training set
    ...
    for i in range(m):
        ...
        trainingMat[i] = img2vector('../../input/knn/trainingDigits/%s' % fileNameStr)

    # 2. 导入测试数据
    testFileList = os.listdir('../../input/knn/trainingDigits')  # iterate through the test set
    errorCount = 0
    ...
    for i in range(mTest):
        ...
        fileStr = fileNameStr.split('.')[0]  # take off .txt
        classNumStr = int(fileStr.split('_')[0])
        vectorUnderTest = img2vector('../input/knn/trainingDigits/%s' % fileNameStr)
