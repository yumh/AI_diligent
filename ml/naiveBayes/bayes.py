...
def spam_test():
    ...
    for i in range(1, 26):
        ...
        try:
            words = text_parse(open('../../input/naiveBayes/email/spam/{}.txt'.format(i)).read())
        except:
            words = text_parse(open('../../input/naiveBayes/email/spam/{}.txt'.format(i), encoding='Windows 1252').read())
        doc_list.append(words)
        full_text.extend(words)
        class_list.append(1)
        try:
            # 添加非垃圾邮件
            words = text_parse(open('../../input/naiveBayes/email/ham/{}.txt'.format(i)).read())
        except:
            words = text_parse(open('../../input/naiveBayes/email/ham/{}.txt'.format(i), encoding='Windows 1252').read())
        doc_list.append(words)
        full_text.extend(words)
        class_list.append(0)
    # 创建词汇表
